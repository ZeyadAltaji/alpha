...
class ForgetStamps extends StatefulWidget {
  const ForgetStamps({Key? key}) : super(key: key);

  @override
  State<ForgetStamps> createState() => _ForgetStampsState();
}
...
class _ForgetStampsState extends State<ForgetStamps> {
  @override
  void initState() {
    super.initState();
    fDate = now;
    tDate = now;
    if (forgetStampsData == null)
      fire = false;
    else
      fire = true;
  }
    ...
    child: Container(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Text(dayName(DateTime.parse(data['sDate']))),
              SizedBox(
                width: 15,
              ),
              Text(dateFormat.format(DateTime.parse(data['sDate']))),
            ],
          ),
          Row(
            children: [
            ...
            children: [
              IconButton(
                onPressed: data['canRequest'] != 0
                    ? null
                    : () {
                        manage(context!, data['sDate'], data['empNo'], (bool t) {
                          onDone!(t);
                        });
                      },
                icon: Icon(
                  ...
                  showCupertinoModalBottomSheet(
                    ...
                    builder: (BuildContext context) {
                      return AbsenceRequestDetails(
                        ...
                        compNo: data['compNo'],
                        sDate: DateTime.parse(data['sDate']),
                      );
                    },
                  );
                },
...
Future<List?> getAbsences(DateTime fDate, DateTime tDate) async {
  if (forgetStampsData != null) return forgetStampsData!;
